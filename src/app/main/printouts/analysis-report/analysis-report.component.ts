...
export class AnalysisReportComponent implements OnInit, OnDestroy {
	...
	exams: any[] = [];
	selected: any = {};
	stateparams: { intakeid: number, streamid: number, subjectid: number, seriesid: number, egroupid: number, classid: number, examid: number } = { intakeid: 0, streamid: 0, subjectid: 0, seriesid: 0, egroupid: 0, classid: 0, examid: 0 };
	...
	ngOnInit(): void {
		...
		if (this.items?.intakeid || this.items?.seriesid) {
			this.cparams = {
				classid: this.items?.classid,
				examid: this.items?.examid,
				streamid: this.items?.streamid
			};

			this.stateparams.classid = this.cparams.classid;
			this.stateparams.examid = this.cparams.examid;
	...
	getSchool_profile() {
		this.schoolService.schoolInfo.pipe(takeUntil(this.destroy$)).subscribe(resp => {
			...
			if (this.school_profile?.logo && this.school_profile?.logo?.length > 0) {
	...
	onKcseMeanChange() {
		this.classData.subjects.list.forEach((sb: any) => {
			...
			if (previous_mean == null || previous_mean == undefined || this.selected_kcse_mean.item == null || this.selected_kcse_mean.item == undefined) {
				...
				if (this.classData != null && this.classData.subjects != null) {
	...
	loadData() {
		...
		this.exams = this.classData.exams.reverse();
		...
		if (this.classData.timeseries != undefined) {
			if (this.classData.timeseries.examseries != undefined) {
				...
				this.classData.timeseries.examseries.list.forEach((stream: any) => {
					stream.data.forEach((sd: any) => {
						...
						if (newObject.value == undefined || newObject.value == null || newObject.value < 0) {
							newObject.y = null;
						} else {
					...
					const filtered = newstreamDataObj.filter(({ id }, index) => !ids.includes(id, index + 1));
					// console.warn("newstreamDataObj >> ", newstreamDataObj);
					// console.warn("stream.data >> ", stream.data);
					// console.warn("filtered >> ", filtered);
					this.timeseries_examseries = [...this.timeseries_examseries, {
						name: stream.streamname,
						colorByPoint: false,
						cursor: "pointer",
						type: "line",
			...
			if (handle_kcse_mean_time_series) {
				...
				if (this.classData.kcse_means_previous != undefined && this.classData.kcse_compare) {
		...
		this.fetchDataCount++;
		console.warn("this.fetchDataCount >> ", this.fetchDataCount);
		this.show_data = true;
	...
	fetchContent(spreadsheet = false) {
		...
		console.warn("this.fetchDataCount fetchContent >> ", this.fetchDataCount);
		let params = "";
		if (this.fetchDataCount == 0) {
			...
			// // console.warn("Second if", this.selected);
			// if ((this.selected.stream?.streamid != undefined && this.selected.stream?.streamid != null && this.selected.stream?.streamid > 0) || (this.selected_stream?.streamid != undefined && this.selected_stream?.streamid != null && this.selected_stream?.streamid > 0)) {
			// 	params = "?streamid=" + (this.selected.stream?.streamid || this.selected_stream?.streamid);
			// } else if ((this.selected.stream?.intakeid != undefined && this.selected.stream?.intakeid != null && this.selected.stream?.intakeid > 0) || (this.selected_stream?.intakeid != undefined && this.selected_stream?.intakeid != null && this.selected_stream?.intakeid > 0)) {
			// 	params = "?intakeid=" + (this.selected.stream?.intakeid || this.selected_stream?.intakeid);
			// }

			// console.warn("Second if", this.selected);
			if ((this.selected.stream?.streamid != undefined && this.selected.stream?.streamid != null && this.selected.stream?.streamid > 0)) {
				params = "?streamid=" + (this.selected.stream?.streamid);
			} else if ((this.selected.stream?.intakeid != undefined && this.selected.stream?.intakeid != null && this.selected.stream?.intakeid > 0)) {
				params = "?intakeid=" + (this.selected.stream?.intakeid);
			} else {
				if (this.stateparams.streamid > 0) {
					params = "?streamid=" + this.stateparams.streamid;
				} else if (this.stateparams.intakeid > 0) {
			...
			if (this.selected.exam.seriesid != undefined && this.selected.exam.seriesid != null && this.selected.exam.seriesid > 0) {
				params += "&seriesid=" + this.selected.exam.seriesid;
			} else if (this.selected.exam.egroupid != undefined && this.selected.exam.egroupid != null && this.selected.exam.egroupid > 0) {
				params += "&egroupid=" + this.selected.exam.egroupid;
			}
			// console.warn("else params >> ", params)
		}
		...
		if (spreadsheet) {
			this.studentsService.getStreamIntakeExamData(params, false, true).subscribe({
				next: (resp_success: any) => {
					const blob = new Blob([resp_success], { type: "application/vnd.ms-excel" });
					this.printoutsService.custom_saver(blob, `${this.classData.classname}_${this.classData.examname}_Analysis_Report_${new Date().getTime()}`);
				},
				error: error => {
					console.error(error);
					const message = this.translate.instant("exams.classAnalysis.toastMessages.fetchContentError");
			...
			this.studentsService.getStreamIntakeExamData(params, false).subscribe((resp: any) => {
				this.classData = resp;
				this.loaded = true;
				this.sortedData = this.classData.subjects.list.slice();
				this.loadData();
				this.classData.aggregate_stats.entries > 0 ? this.no_data = false : this.no_data = true;
				this.fetching_examsData_inprogress = false;
			});
		}
	}
	...
	fetchIntakeStreamExams(intake: any, stream: any, seriesid: any = null, egroupid: any = null) {
		let params = "";
		let has_state_params = false;
		let has_selected_exam = false;
		if ((seriesid != null && seriesid.length > 0) || (egroupid != null && egroupid.length > 0)) {
				...
				if (this.exams == null || this.exams.length == 0) {
					...
					if (has_state_params || has_selected_exam) {
						this.exams.forEach((exam: any) => {
							if (seriesid.length > 0 && exam.seriesid != null && exam.seriesid == seriesid) {
								this.selected.exam = exam;
							} else if (egroupid.length > 0 && exam.egroupid != null && exam.egroupid == egroupid) {
								this.selected.exam = exam;
							}
						});
					}
	...
	getAnalysisReport() {
		...
		// console.log(this.selected.stream)
		// console.log(this.selected.stream.streamid)

		if (this.selected.stream != undefined && this.selected.stream != null && this.selected.stream.streamid != undefined && this.selected.stream.streamid != null && this.selected.stream.streamid > 0) {
	...
	sortData(sort: Sort) {
		...
		this.sortedData = data.sort((a: any, b: any) => {
			...
			case "previous": return compare(a.previous, b.previous, isAsc);
