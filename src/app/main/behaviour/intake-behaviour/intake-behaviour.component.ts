...
@Component({
	selector: "app-intake-behaviour",
	templateUrl: "./intake-behaviour.component.html",
	styleUrls: ["./intake-behaviour.component.scss",
		"../class-behaviour/class-behaviour.component.scss"]
})
export class IntakeBehaviourComponent implements OnInit {
	...
	ngOnInit(): void {
		this.route.params.subscribe((params) => {
			...
			this.loadBehaviourTimeLine("?type=1");
		});
	}
	...
	loadBehaviourTimeLine(param: any) {
		this.bService.getBehaviourTimeline(param).subscribe(
			(res) => {
				...
				const message = this.translate.instant("behaviour.intakeBehavoiur.toastMessages.loadBehaviourTimeline.error");
	...
	loadStatistics() {
		...
		this.bService.getClassStatistics(this.selectedYear, this.selectedTerm, this.selectedType,this.pathParams.intakeid,null).subscribe(
			(res) => {
				this.classStatistics = res;
			}, (err) => {
				const message = this.translate.instant("behaviour.intakeBehavoiur.toastMessages.loadStatisctics.error");
				this.toastService.error(message);
			}
		);
	}
	loadClassRecent(page: any) {
		this.bService.getClassRecent(this.selectedYear, this.selectedTerm, this.selectedType, page,this.pathParams.intakeid,null,null).subscribe(
			(res) => {
				this.classRecent = res;
			}, (err) => {
				const message = this.translate.instant("behaviour.intakeBehavoiur.toastMessages.loadClassRecent.error");
				this.toastService.error(message);
			}
		);
	}
	loadClassStudentPoints(page: any) {
		this.bService.getClassStudentPoints(this.selectedYear, this.selectedTerm, this.selectedType, page,this.pathParams.intakeid,null,null).subscribe(
			(res) => {
				this.classStudentPoints = res;
			}, (err) => {
				const message = this.translate.instant("behaviour.intakeBehavoiur.toastMessages.loadClassStudentPoints.error");
				this.toastService.error(message);
			}
		);
	}
	loadAwards(page: any) {
		this.bService.getClassStudentAwards(this.selectedYear, this.selectedTerm, this.selectedType, page, this.pathParams.intakeid,null,null).subscribe(
			(res) => {
				this.classStudentAwards = res;
			}, (err) => {
				const message = this.translate.instant("behaviour.intakeBehavoiur.toastMessages.loadAwards.error");
				this.toastService.error(message);
			}
		);
	}
